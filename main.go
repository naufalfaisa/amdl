...
func getUrlArtistName(artistUrl string, token string) (string, string, error) {
	storefront, artistId := helpers.CheckURLArtist(artistUrl)
...
func buildFFmpegArgs(inPath, outPath, targetFmt, extraArgs string) ([]string, error) {
...
func ripTrack(track *task.Track, token string, mediaUserToken string) {
	...
	if needCheck && !needDlAacLc {
		EnhancedHls_m3u8, _ = media.CheckM3u8(track.ID, "song", Config.GetM3u8FromDevice, Config.GetM3u8Port)
	...
	if strings.Contains(Config.SongFileFormat, "Quality") {
		if dl_atmos {
			...
			_, Quality, err = media.ExtractMedia(track.M3u8, true, dl_atmos, dl_aac, Config)
			if err != nil {
	...
	filename := fmt.Sprintf("%s.m4a", helpers.ForbiddenNames.ReplaceAllString(songName, "_"))
	track.SaveName = filename
	trackPath := filepath.Join(track.SaveDir, track.SaveName)
	lrcFilename := fmt.Sprintf("%s.%s", helpers.ForbiddenNames.ReplaceAllString(songName, "_"), Config.LrcFormat)
	...
	if needDlAacLc {
		...
		trackM3u8Url, _, err := media.ExtractMedia(track.M3u8, false, dl_atmos, dl_aac, Config)
		if err != nil {
...
func ripStation(albumId string, token string, storefront string, mediaUserToken string) error {
	...
	singerFolder := filepath.Join(Config.AlacSaveFolder, helpers.ForbiddenNames.ReplaceAllString(singerFoldername, "_"))
	if dl_atmos {
		singerFolder = filepath.Join(Config.AtmosSaveFolder, helpers.ForbiddenNames.ReplaceAllString(singerFoldername, "_"))
	}
	if dl_aac {
		singerFolder = filepath.Join(Config.AacSaveFolder, helpers.ForbiddenNames.ReplaceAllString(singerFoldername, "_"))
	}
	...
	playlistFolderPath := filepath.Join(singerFolder, helpers.ForbiddenNames.ReplaceAllString(playlistFolder, "_"))
	...
	if Config.SaveAnimatedArtwork && meta.Data[0].Attributes.EditorialVideo.MotionSquare.Video != "" {
		...
		motionvideoUrlSquare, err := media.ExtractVideo(meta.Data[0].Attributes.EditorialVideo.MotionSquare.Video, 9999)
		if err != nil {
			fmt.Println("no motion video square.\n", err)
		} else {
			exists, err := helpers.FileExists(filepath.Join(playlistFolderPath, "square_animated_artwork.mp4"))
			if err != nil {
	...
	if station.Type == "stream" {
		...
		fmt.Println(songName)
		trackPath := filepath.Join(playlistFolderPath, fmt.Sprintf("%s.m4a", helpers.ForbiddenNames.ReplaceAllString(songName, "_")))
...
func ripAlbum(albumId string, token string, storefront string, mediaUserToken string, urlArg_i string) error {
	...
	if debug_mode {
		...
		for trackNum, track := range meta.Data[0].Relationships.Tracks.Data {
			...
			if needCheck {
				fullM3u8Url, err := media.CheckM3u8(track.ID, "song", Config.GetM3u8FromDevice, Config.GetM3u8Port)
			...
			_, _, err = media.ExtractMedia(m3u8Url, true, dl_atmos, dl_aac, Config)
			if err != nil {
		...
		if dl_atmos {
			...
			if err != nil {
				...
				if manifest1.Data[0].Attributes.ExtendedAssetUrls.EnhancedHls == "" {
					...
					_, Quality, err = media.ExtractMedia(manifest1.Data[0].Attributes.ExtendedAssetUrls.EnhancedHls, true, dl_atmos, dl_aac, Config)
					if err != nil {
						fmt.Println("Failed to extract quality from manifest.\n", err)
					}
				}
	...
	albumFolderPath := filepath.Join(singerFolder, helpers.ForbiddenNames.ReplaceAllString(albumFolderName, "_"))
	...
	if Config.SaveAnimatedArtwork && meta.Data[0].Attributes.EditorialVideo.MotionDetailSquare.Video != "" {
		...
		motionvideoUrlSquare, err := media.ExtractVideo(meta.Data[0].Attributes.EditorialVideo.MotionDetailSquare.Video, 9999)
		if err != nil {
			fmt.Println("no motion video square.\n", err)
		} else {
			exists, err := helpers.FileExists(filepath.Join(albumFolderPath, "square_animated_artwork.mp4"))
			if err != nil {
				fmt.Println("Failed to check if animated artwork square exists.")
			}
			if exists {
		...
		motionvideoUrlTall, err := media.ExtractVideo(meta.Data[0].Attributes.EditorialVideo.MotionDetailTall.Video, 9999)
		if err != nil {
			fmt.Println("no motion video tall.\n", err)
		} else {
			exists, err := helpers.FileExists(filepath.Join(albumFolderPath, "tall_animated_artwork.mp4"))
			if err != nil {
...
func ripPlaylist(playlistId string, token string, storefront string, mediaUserToken string) error {
	...
	if Config.SaveAnimatedArtwork && meta.Data[0].Attributes.EditorialVideo.MotionDetailSquare.Video != "" {
		...
		if err != nil {
			...
			exists, err := helpers.FileExists(filepath.Join(playlistFolderPath, "tall_animated_artwork.mp4"))
			if err != nil {
				fmt.Println("Failed to check if animated artwork tall exists.")
			}
			if exists {
	...
	for {
		for albumNum, urlRaw := range os.Args {
			...
			if strings.Contains(urlRaw, "/music-video/") {
				...
				if mvSaveDir != "" {
					mvSaveDir = filepath.Join(Config.AlacSaveFolder, helpers.ForbiddenNames.ReplaceAllString(mvSaveDir, "_"))
				} else {
...
func mvDownloader(adamID string, saveDir string, token string, storefront string, mediaUserToken string, track *task.Track) error {
	...
	mvOutPath := filepath.Join(saveDir, fmt.Sprintf("%s.mp4", helpers.ForbiddenNames.ReplaceAllString(mvSaveName, "_")))
	...
	videom3u8url, _ := media.ExtractVideo(mvm3u8url, Config.MVMax)
	videokeyAndUrls, _ := runv3.Run(adamID, videom3u8url, token, mediaUserToken, true, "")
	_ = runv3.ExtMvData(videokeyAndUrls, vidPath)
	defer os.Remove(vidPath)
	audiom3u8url, _ := media.ExtractMvAudio(mvm3u8url, Config.MVAudioType)
	...
	if true {
		...
		baseThumbName := helpers.ForbiddenNames.ReplaceAllString(mvSaveName, "_") + "_thumbnail"
