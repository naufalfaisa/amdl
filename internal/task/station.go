...
func (a *Station) GetResp(mutoken, token, l string) error {
	var err error
	a.Language = l
	resp, err := api.GetStationResp(a.Storefront, a.ID, a.Language, token)
	if err != nil {
	...
	tracksResp, err := api.GetStationNextTracks(a.ID, mutoken, a.Language, token)
	if err != nil {
	...
	for i, trackData := range tracksResp.Data {
		albumResp, err := api.GetAlbumRespByHref(trackData.Href, a.Language, token)
		if err != nil {
